...
def add_tutorial(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Failed to upload tutorial. \
                Please check the form is valid and try again.')
    else:
        form = TutorialForm()

    template = 'tutorial/add_tutorial.html'
    context = {
        'form': form,
    }
...
def edit_tutorial(request, tutorial_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Your tutorial has been \
    ...
    template = 'tutorial/edit_tutorial.html'
    context = {
        'form': form,
        'tutorial': tutorial,
    }
...
def delete_tutorial(request, tutorial_id):
    ...
    return redirect(reverse('tutorial'))
