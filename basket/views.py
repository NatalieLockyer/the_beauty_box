    ...
    if shades:
        if item_id in list(basket.keys()):
            if shades in basket[item_id]['items_by_shades'].keys():
                ...
                messages.success(
                    request,
                    (
                        f'Added shade {shades} {product.name} to your basket')
                    )
        else:
            basket[item_id] = {'items_by_shades': {shades: quantity}}
            messages.success(
        ...
        if item_id in list(basket.keys()):
            ...
            messages.success(
                request,
                (
                    f'This item "{product.name}" has been'
        ...
        if quantity > 0:
            ...
            if not basket[item_id]['items_by_shades']:
                ...
                messages.success(
                    request,
                    (
                        f'Removed shade {shades} {product.name}'
                        f' to your basket')
                    )
    else:
        if quantity > 0:
            ...
            messages.success(
                request,
                (
                    f'This item "{product.name}" has been removed'
                    f' from your basket')
                )
